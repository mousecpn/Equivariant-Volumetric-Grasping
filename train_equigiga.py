...
from model.equi_giga import TriEquiGIGA, EquiGIGA3d
import time
...
def _qual_loss_fn(pred, target):
    return binary_cross_entropy_with_label_smoothing(pred, target, reduction="none", smoothing=0.1)
...
def _occ_loss_fn(pred, target):
    return binary_cross_entropy_with_label_smoothing(pred, target, reduction="none", smoothing=0.1).mean(-1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--logdir", type=Path, default="data/equigiga_runs")
